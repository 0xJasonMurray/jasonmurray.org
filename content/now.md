---
title: "What am I doing right NOW?"
date: 2021-01-03T14:09:41-06:00
description: What am I doing right now?
---

# Work

- Working for [Washington University in St. Louis](https://wustl.edu/) as a Network Architect:
  - Upgrading the university network edge to 100G, adding direct 10G connections to AWS, Azure, and GCP, and providing private peerings to major content providers.
  - Leading the project to evaluate replacing our client and data center networks with a software defined architecture.
  - Designing a parallel network to be NIST-800-171 compliant.
  - Working to support our medical, research, and students so they can focus on studies, treating patients, and finding solutions for the COVID-19 crisis.

# Family

- Raising 2 teenage kids:
  - Starting the process of university selections for the oldest.
  - Helping my youngest succeed as a freshman in high school.
  - Teaching my 15 year old to drive.
- Supporting my wife (and family) as she battles stage 4 breast cancer:
  - Right now everything is mostly normal.  She is receiving chemo therapy every week and tolerating it fairly well.  
- My son and I are preparing for a 100 mile backpacking trip with the Boy Scouts at Philmont in 2022.

# Hobbies

- Run, walk, or strength train everyday.
  - I cut back running significantly because of Osgood-Schlatter disease. PT and limited miles is slowly helping, but not fast enough. Running was a big part of my daily routine.
- Reading or listening to books:
  - Currently reading:
    - Being Geek
    - Death by Meeting: A Leadership Fable
  - Recently finished (2021):
    - Evil Geniuses: The Unmaking of America
    - The Darkest Web (far too dark, most people don't need to know this exists on the Internet)
    - Wild
    - Dark Matter
    - Ready Player Two
    - Hitchhikers Guide to the Galaxy (No, I have never read this book before)
    - American Kingpin
    - Recursion
    - Why Buddhism is True
    - Being Mortal
    - A Promised Land
    - Bossy Pants
    - Becoming
- Nerdy stuff:
  - My hobby life revolves around Internet technology.  
  - I spend a lot of time tinkering in my [home lab](/posts/2020/homelab/).
- Writing something for this site when the mood hits me.