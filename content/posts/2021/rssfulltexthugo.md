...
By default, [Hugo](https://gohugo.io/) summarizes each article when generating the RSS feed. This is not ideal, if your the type of person who prefers to read the full content directly in an RSS reader.  This post will show you how to enable full text RSS feeds in Hugo.
...
Then copy the following file into the `layouts/_default` directory.  It's is a modified version of the default [RSS template](https://raw.githubusercontent.com/gohugoio/hugo/master/tpl/tplimpl/embedded/templates/_default/rss.xml) with the appropriate modifications to generate full text RSS feeds:
